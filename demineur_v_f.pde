...
void draw () {
  background(192);
  display ();
  //on change les coordonnées pour que les elements soient dans la case
  //drawBloc((colonnes/2)-1,(lignes/2)-1,cote,cote);
  //drawBomb((colonnes/2)-1,(lignes/2)-1);
  
  //if ( etat == started){
    //le temps est obtenu en faisant la différence entre le moment actuel (millis) et le début.
    // la division par 1000 convertit de millisecondes à secondes
    // on définit ce calcul avec un entier pour avoir seulement le résultat en secondes entières ( comme dans le jeu)
    //time = int((millis()-start) / 1000); 
  //}
  drawTime();
  drawScore();
  //afficher le temmps pour verrifier
  //println(time);
  //afficher les chiffres pour verrifier  le bon fonctionnement
  //println(etat);
  if(etat == started)
...
void display () {
  ...
  textFont(nombres);
  //modification pour afficher le bloc au centre de l'interface du bandeau ( derrière le smiley) PROBLEME : le bloc ne s'affiche pas
  //drawBloc(width/2-(cote/2), bandeau/2, cote,cote);
  //drawBloc(width/2-10, bandeau/2, 20,20);
  ...
  /*
  if (etat_bloc == bloc || etat_bloc ==flag){ // afficher un bloc si on est dans l'état bloc ou flag
    drawBloc ( colonnes/2 , lignes/2 - 1 , 0,0);
    if (etat_bloc == flag ){ //afficher un drapeau si dans l'état flag
      drawFlag ( colonnes/2, lignes/2 - 1);
    }
  }
  */
